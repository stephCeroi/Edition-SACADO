...
\index{Fonction!Antécédent}\index{Fonction!}\index{Fonction!Ensemble de définition}

\begin{minipage}{0.48\linewidth}
Définir une fonction $f$ d'un ensemble $\mathcal{D}$ de réels dans $\mathbb{R}$, c'est associer à chaque réel $x$ de $\mathcal{D}$ un
...
\begin{enumerate}
\item $\mathcal{D}_f$ est centré en $0$.
\item pour tout $x \in \mathcal{D}_f$, $f(x)=f(-x)$.
\end{enumerate}
\end{Mt}
\end{minipage}
